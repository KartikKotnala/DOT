    ...
    >
      <div class="bg-panel border border-line w-full max-w-md rounded-3xl p-6 shadow-2xl relative flex flex-col gap-5 transform scale-100 transition-transform">
        ...
        <button 
          ...
          class="absolute top-4 right-4 text-muted hover:text-main text-sm font-bold font-mono transition-colors focus:outline-none cursor-pointer"
        >
          ✕
        </button>
        ...
        <div class="mt-2 flex gap-3">
          ...
          <button 
            ...
            class="flex-1 py-3 bg-red-600 hover:bg-red-500 text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-200 shadow-md transform active:scale-[0.99] cursor-pointer focus:outline-none"
          >
            Lock Waypoint Route
          </button>
        </div>
        ...
        <div class="flex-1 w-full flex flex-col gap-4 h-full relative">
          ...
          >
            ...
            <div class="w-full h-full relative overflow-hidden bg-white">
              ...
              >
                <svg data-target-map ref="svgContainerElement" @click="handleMapWorkspaceClick($event)" class="w-[800px] h-[600px] shrink-0" viewBox="0 0 800 600" xmlns="http://www.w3.org/2000/svg">
...
// 🚀 LIVE BACKEND API CALLS (PORT 5001)
onMounted(async () => {
  try {
    ...
    console.error("Failed to load map configurations from Port 5001:", error);
  }
});
